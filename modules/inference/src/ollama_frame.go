...
func (infer *Inference) GenerateOllamaResource(request *module.GeneratorRequest) ([]apiv1.Resource, *apiv1.Patcher, error) {
	...
	svc, _, err := infer.generateOllamaService(request)
	if err != nil {
		return nil, nil, err
	}
	...
	deploymentProxy, err := infer.generateProxyDeployment(request)
	if err != nil {
		return nil, nil, err
	}
...
func (infer *Inference) generateProxyPodSpec(_ *module.GeneratorRequest) (v1.PodSpec, error) {
...
func (infer *Inference) generateProxyDeployment(request *module.GeneratorRequest) (*apiv1.Resource, error) {
	podSpec, err := infer.generateProxyPodSpec(request)
	if err != nil {
		return nil, nil
	}
